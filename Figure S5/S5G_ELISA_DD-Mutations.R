...
settings <- elisa_analysis_setup(
  main_path = "~/Desktop/2_Source files/Figure S4/S4G_ELISA_DD-Mutations/",
...
if (normalize_data) {
  ...
  DD_Mutations <- plate_data %>% filter(Date == "2024-12-02", STIM_DAY %in% c(4:6))
  DD_Mutations_normalized <- process_ELISA_data(DF = DD_Mutations, NEGATIVE_CTRL = "cl204", POSITIVE_CTRL = "cl232") %>% 
    filter(CL_NUMBER != "cl069")
  
  plotting_data  <- rbind(DD_Mutations_normalized)
...
result <- prepare_elisa_plot(plotting_data, method = "v1")
