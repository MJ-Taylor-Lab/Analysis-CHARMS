...
settings <- elisa_analysis_setup(
  main_path = "~/Desktop/2_Source files/Figure 3/3F_ELISA_Poly-xTA/",
...
if (normalize_data) {
  ...
  plotting_data <- bind_rows(AMYLOIDS_20240822_normalized %>% 
                               filter(CL_NUMBER %in% c("cl489", "cl490", # 17xTA , 20xTA
                                                       "cl069", "cl204", # Positive + Negative control
                                                       "cl486",          # 11xTA
                                                       "cl488")))        # 15xTA
} else {
