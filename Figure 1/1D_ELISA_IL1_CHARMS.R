...
if (normalize_data) {
  ...
  CHARMS_data       <- CHARMS_normalized %>% filter(CL_NUMBER %in% c("cl232", "cl244"), Date == "2022-06-23")
  
  CHARMS_3xA_data <- CHARMS_normalized %>% filter(CL_NUMBER %in% c("cl234"), Date == "2022-07-01")
  
  CHARMS_3xKO           <- plate_data %>% filter(Date == "2022-06-09")
  CHARMS_3xKO_nomalized <- process_ELISA_data(DF = CHARMS_3xKO, NEGATIVE_CTRL = "cl204", POSITIVE_CTRL = "cl011")
  CHARMS_3xKO_data      <- CHARMS_3xKO_nomalized %>% filter(CL_NUMBER %in% c("cl204", "cl011"))
  
  plotting_data  <- rbind(CHARMS_data, CHARMS_3xKO_data, CHARMS_3xA_data)
} else {
